...
export function parseResultsEnvelope(value: unknown): ResultsEnvelope {
  ...
  if (value.schemaVersion === "2.0") {
    if (value.kind === "run" && isRecordedSimulationRun(value.run)) {
      return { schemaVersion: "2.0", kind: "run", run: value.run };
    }
    if (
      value.kind === "run-list" && Array.isArray(value.runs) &&
      value.runs.every(isRecordedRunSummary)
    ) {
      return { schemaVersion: "2.0", kind: "run-list", runs: value.runs };
    }
  } else {
    if (value.kind === "run" && isLegacySimulationRun(value.run)) {
...
function isRecordedSimulationRun(value: unknown): value is SimulationRun {
  if (!isRecordedRunSummary(value) || !isRecord(value)) return false;
  return isEngine(value.engine) && isResultNormalizer(value.result_normalizer) &&
    ...
    Array.isArray(value.artifacts) && value.artifacts.every(isRecordedArtifact) &&
...
function isLegacySimulationRun(value: unknown): value is LegacySimulationRun {
  if (!isLegacyRunSummary(value) || !isRecord(value)) return false;
  return isEngine(value.engine) && isQuantityMap(value.resolved_parameters) &&
    isQuantityMap(value.metrics) && Array.isArray(value.artifacts) &&
    value.artifacts.every(isLegacyArtifact) && Array.isArray(value.warnings) &&
...
function isParameterSchema(
  ...
  return isRecord(value) && typeof value.source_sha256 === "string" &&
...
function isLegacyArtifact(value: unknown): value is LegacySimulationRun["artifacts"][number] {
  return isArtifactBase(value) && LEGACY_ARTIFACT_KINDS.includes(
